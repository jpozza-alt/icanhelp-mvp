...
function Get-TenantIdsFromTenantsResponse {
    param([object]$Json)
    ...
    if ($Json -is [System.Array] -or $Json -is [object[]]) {
        foreach ($item in @($Json)) {
            if ($null -eq $item) { continue }

            if ($item.PSObject.Properties.Name -contains "tenant_id") {
    ...
    if ($Json.PSObject.Properties.Name -contains "tenant_id") {
        $value = [string]$Json.tenant_id
        if (-not [string]::IsNullOrWhiteSpace($value)) {
            $ids.Add($value)
        }
    }
    ...
    if (($ids.Count -eq 0) -and ($Json.PSObject.Properties.Name -contains "items")) {
...
$RunDir = Join-Path $RepoRoot ("_debug\cross_tenant_ticket_isolation_no_activation_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
...
try {
    ...
    $primaryTenantsJson = Convert-JsonSafe -Text $primaryTenantsResp.body_text
    $secondaryTenantsJson = Convert-JsonSafe -Text $secondaryTenantsResp.body_text

    $primaryTenantIds = @(Get-TenantIdsFromTenantsResponse -Json $primaryTenantsJson)
    $secondaryTenantIds = @(Get-TenantIdsFromTenantsResponse -Json $secondaryTenantsJson)
    ...
    if ($primaryTenantId -eq $secondaryTenantId) {
        throw "PRIMARY and SECONDARY resolved to the same tenant_id."
    }
    ...
    $createBody = @{
        title = $ticketTitle
        description = "cross tenant isolation no activation"
