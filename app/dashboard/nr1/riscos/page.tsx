...
const FIELD_LABELS: Record<string, string> = {
  "riscos levantados": "Riscos levantados",
  "fontes geradoras": "Fontes geradoras",
  "grupos expostos": "Grupos expostos",
  "prioridade inicial": "Prioridade inicial",
};
...
export default function Nr1RiscosPage() {
  ...
  function updateTextField(field: keyof Nr1RiscosLocalDraft, value: string) {
    setDraft((current) => ({
      ...current,
      [field]: value,
      isCompleted: false,
  ...
  function updateBooleanField(field: keyof Nr1RiscosLocalDraft, value: boolean) {
    setDraft((current) => ({
      ...current,
      [field]: value,
      isCompleted: false,
  ...
  function handleManualSave() {
    ...
    try {
      ...
      setSaveStatus("error");
      setFeedback("Falha ao salvar o rascunho de riscos.");
    }
  }
  ...
  function handleClearDraft() {
    ...
    setSaveStatus("idle");
    setFeedback("Rascunho local de riscos removido.");
  }
  ...
  function handleComplete() {
    ...
    try {
      ...
      setSaveStatus("saved");
      setFeedback("Riscos concluidos localmente.");
    } catch {
      setSaveStatus("error");
      setFeedback("Falha ao concluir a etapa de riscos.");
    }
  }
  ...
  return (
    <Nr1StepGuard
      stepKey="riscos"
      title="Riscos"
      description="Levante os riscos principais antes de liberar o plano de acao."
    >
        ...
        <div className="mx-auto max-w-[1400px] px-6 py-8">
          <section className="overflow-hidden rounded-[28px] bg-[linear-gradient(135deg,#0F2337_0%,#13495C_60%,#178A8F_100%)] p-7 text-white shadow-[0_10px_30px_rgba(18,40,70,0.08)]">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
              <div className="max-w-[760px]">
                <div className="text-[12px] uppercase tracking-[0.08em] text-white/70">etapa de analise</div>
                <h1 className="mt-4 text-[38px] font-semibold leading-tight">Riscos</h1>
                <p className="mt-3 text-base leading-7 text-white/85">
                  Esta etapa deixa de ser apenas um bloco de notas. Agora ela registra riscos,
                  fontes geradoras, grupos expostos e uma prioridade inicial para liberar o plano.
                </p>
              </div>
              ...
              <div className="grid min-w-[300px] gap-3 rounded-[22px] border border-white/20 bg-white/10 p-4 backdrop-blur">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <strong id="nr1RiscosProgressPercent">{progress}%</strong>
                </div>
                ...
                <div className="text-sm text-white/85">
                  Conclusao formal:{" "}
                  <strong id="nr1RiscosCompletedAt">
          ...
          <section className="mt-[18px] grid gap-[18px] lg:grid-cols-2">
            <div className="rounded-[24px] border border-[#DBE5F0] bg-white p-6 shadow-[0_10px_30px_rgba(18,40,70,0.08)]">
              <h2 className="text-2xl font-semibold">Levantamento inicial</h2>
              <p className="mt-3 text-sm leading-7 text-[#60718A]">
                Registre os riscos percebidos, suas fontes e quem esta exposto antes de discutir medidas.
              </p>

              <div className="mt-6 grid gap-4">
                <Field
                  id="riscosLevantados"
                  label="Riscos levantados"
                  value={draft.riscosLevantados}
                  onChange={(value) => updateTextField("riscosLevantados", value)}
                  placeholder="Ex.: sobrecarga, repeticao, conflito com publico, risco de acidente..."
                  multiline={true}
                />
                <Field
                  id="fontesGeradoras"
                  label="Fontes geradoras"
                  value={draft.fontesGeradoras}
                  onChange={(value) => updateTextField("fontesGeradoras", value)}
                  placeholder="Ex.: demanda, postura, deslocamento, maquinas, prazos..."
                  multiline={true}
                />
                <Field
                  id="gruposExpostos"
                  label="Grupos expostos"
                  value={draft.gruposExpostos}
                  onChange={(value) => updateTextField("gruposExpostos", value)}
                  placeholder="Ex.: administrativo, atendimento, operacional, externo..."
                  multiline={true}
                />
                <Field
                  id="prioridadeInicial"
                  label="Prioridade inicial"
                  value={draft.prioridadeInicial}
                  onChange={(value) => updateTextField("prioridadeInicial", value)}
                  placeholder="Ex.: alta, media, baixa ou ordem de atacacao."
                />
              </div>
            </div>

            <div className="rounded-[24px] border border-[#DBE5F0] bg-white p-6 shadow-[0_10px_30px_rgba(18,40,70,0.08)]">
              <h2 className="text-2xl font-semibold">Leitura complementar</h2>
              <p className="mt-3 text-sm leading-7 text-[#60718A]">
                Registre as medidas existentes e sinalize a natureza principal dos riscos observados.
              </p>

              <div className="mt-6 grid gap-3">
                <Toggle
                  id="possuiRiscoPsicossocial"
                  label="Ha risco psicossocial relevante nesta leitura?"
                  checked={draft.possuiRiscoPsicossocial}
                  onChange={(value) => updateBooleanField("possuiRiscoPsicossocial", value)}
                />
                <Toggle
                  id="possuiRiscoErgonomico"
                  label="Ha risco ergonomico relevante nesta leitura?"
                  checked={draft.possuiRiscoErgonomico}
                  onChange={(value) => updateBooleanField("possuiRiscoErgonomico", value)}
                />
                <Toggle
                  id="possuiRiscoAcidente"
                  label="Ha risco de acidente relevante nesta leitura?"
                  checked={draft.possuiRiscoAcidente}
                  onChange={(value) => updateBooleanField("possuiRiscoAcidente", value)}
                />
                <Field
                  id="medidasExistentes"
                  label="Medidas existentes"
                  value={draft.medidasExistentes}
                  onChange={(value) => updateTextField("medidasExistentes", value)}
                  placeholder="Ex.: pausas, EPI, divisao de tarefas, orientacoes, barreiras..."
                  multiline={true}
                />
                <Field
                  id="observacoesRiscos"
          ...
          <section className="mt-[18px] rounded-[24px] border border-[#DBE5F0] bg-white p-6 shadow-[0_10px_30px_rgba(18,40,70,0.08)]">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <p className="mt-3 text-sm leading-7 text-[#60718A]">
                  Este corte de riscos exige os campos minimos abaixo para liberar o plano de acao.
                </p>
