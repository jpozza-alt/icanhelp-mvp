...
export default function Nr1WorkspacePage() {
  ...
  const actions = useMemo(() => {
    const query = contextSaved
      ? "?company=" + encodeURIComponent(company) + "&establishment=" + encodeURIComponent(establishment)
      : "";

    return [
      { title: "Diagnostico inicial", href: "/dashboard/nr1/diagnostico-inicial" + query, text: "Abrir o ponto inicial da jornada autenticada." },
      { title: "Setores e atividades", href: "/dashboard/nr1/setores" + query, text: "Ir direto para a base operacional." },
      { title: "Riscos e prioridades", href: "/dashboard/nr1/riscos" + query, text: "Abrir a etapa de riscos e prioridades." },
    ];
        ...
        <section className="mt-[18px] grid gap-[18px] lg:grid-cols-[1.05fr_0.95fr]">
          <div className="rounded-[24px] border border-[#DBE5F0] bg-white p-6 shadow-[0_10px_30px_rgba(18,40,70,0.08)]">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className={contextReady ? "rounded-full border border-[#C8F0DA] bg-[#EBFBF3] px-[10px] py-[7px] text-xs font-bold text-[#20865A]" : "rounded-full border border-[#FFE3AA] bg-[#FFF8EA] px-[10px] py-[7px] text-xs font-bold text-[#C88A16]"}>
