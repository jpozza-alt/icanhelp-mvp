-- Administrative repair for one diagnosis-generated risk and its linked review.
-- This migration only defines an RPC. It does not change RLS, Auth, tenant data or environment.

create or replace function public.nr1_admin_correct_diagnosis_risk_texts(
  p_tenant_id uuid,
...
revoke all on function public.nr1_admin_correct_diagnosis_risk_texts(
  uuid, uuid, uuid, uuid, uuid, text, text, text, timestamptz, jsonb,
  timestamptz, text, text, text, text, uuid, text
) from public, anon, authenticated;

grant execute on function public.nr1_admin_correct_diagnosis_risk_texts(
  uuid, uuid, uuid, uuid, uuid, text, text, text, timestamptz, jsonb,
  timestamptz, text, text, text, text, uuid, text
) to service_role;
